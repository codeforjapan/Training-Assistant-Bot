...
export function hasRoles(
  ...
  const attachedRoles = Reflect.getMetadata('roles', route);
  const notAttachedRoles = roles.filter(
    (role) => !attachedRoles.includes(role),
  );
  if (notAttachedRoles.length > 0) {
    throw Error(
      `Expected: ${route.name} to have roles: ${notAttachedRoles}\nReceived: ${attachedRoles}`,
    );
  }
  return true;
}
